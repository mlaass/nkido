...
The coerce only fires for monophonic, non-sample patterns. **Polyphonic patterns** (chord, multi-voice) error E160 with a hint to `poly()` — silently emitting voice 0 is rarely what the user wants. **Sample patterns** route through `SAMPLE_PLAY` and produce audio output rather than a scalar; use `out()` directly.
...
`scalar()` is **idempotent** on Signal — `scalar(scalar(p))` is safe. It errors **E161** on sample / polyphonic patterns.

## Pipe-binding stays Pattern
